...
num_vocab=128256
embed_table_path="project_data/sim_table/Llama-3.2-3B.mmap"
sim_table = np.memmap(embed_table_path, dtype=np.float16, mode='r', shape=(num_vocab, num_vocab))
...
def compute_similarity(source_token_ids, tgt_token_ids):
    similarities = []
...
def compute_earth_mover_distance(file_name, save_file_name):
    ...
    for item in tqdm(items):
        for span_item in item['matched_spans']:
            ...
            if not target_token_ids or not ref_token_ids:
                continue
            similarity = min(compute_similarity(target_token_ids, ref_token_ids),
                             compute_similarity(ref_token_ids, target_token_ids))
            outputs.append({
...
def find_matched_span(tgt_token_ids: List[int], threshold: float, min_content_word: int,
        ...
        for start in range(len(ref_to_tgt_scores)):
            for end in range(start + min_content_word, len(ref_to_tgt_scores) + 1):
                ...
                if subarray_mean >= threshold:
                    ...
                    tgt_to_ref_score = compute_similarity(tgt_token_ids, cand_token_ids)
...
def find_soft_match(doc: Document, reference_docs: List[RefDocument], min_ngram: int, min_content_word: int,
    ...
    while second_pointer <= len(doc.tokens):
        ...
        for ref_doc in tqdm(reference_docs):
            matched_spans.update(find_matched_span(span_token_ids, threshold, min_content_word, ref_doc))
...
def dj_search_earth_mover(data_path, embed_table_path, output_file, min_ngram, min_content_word, threshold,
    ...
    for t_idx, t_doc in tqdm(enumerate(data), desc='target docs', total=len(data)):
        ...
        output = find_soft_match(tgt_doc, reference_docs, min_ngram, min_content_word, threshold)
        t_doc.update(output)
